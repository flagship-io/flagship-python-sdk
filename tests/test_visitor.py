...
def test_create_visitor():
    fs = Flagship.instance()
    fs.start(Config("my_env_id", "my_api_key", event_handler=None))

    try:
        visitor = fs.create_visitor("Pam", {'isVIP': True})
        assert visitor._visitor_id == 'Pam'
        assert visitor._context['isVIP'] == True
        assert visitor._env_id == "my_env_id"
        assert visitor._api_key == "my_api_key"
        assert visitor._api_client is not None
        assert visitor._api_client._env_id == "my_env_id"
        assert visitor._api_client.api_key == "my_api_key"
...
def test_visitor_synchronize():
    ...
    fs = Flagship.instance()
    fs.start(Config("my_env_id", "", event_handler=None, mode=Config.Mode.API))
    visitor = fs.create_visitor("visitor_1")
    responses.reset()
    responses.add(responses.POST, 'https://decision-api.flagship.io/v1/my_env_id/campaigns/?exposeAllKeys=true&sendContextEvent=false',
                  json=json.loads(json_response), status=200)
    responses.add(responses.POST, 'https://decision-api.flagship.io/v1/my_env_id/events', status=200)
...
def test_visitor_get_modification():
    ...
    responses.add(responses.POST,
                  'https://decision-api.flagship.io/v1/my_env_id/events', status=200)
    responses.add(responses.POST, 'https://decision-api.flagship.io/v1/activate', status=200)
...
def test_visitor_get_modification2():
    ...
    assert visitor.get_modification_data("btn-color").value == 'red'
    assert visitor.get_modification_data("do_not_exists") is None


@responses.activate
def test_visitor_get_activate():
    fs = Flagship.instance()
    fs.start(Config("my_env_id", "", event_handler=None))
    visitor = fs.create_visitor("visitor_1")
